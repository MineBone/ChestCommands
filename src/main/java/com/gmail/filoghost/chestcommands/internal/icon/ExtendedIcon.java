...
public class ExtendedIcon extends Icon {
    ...
    public double getAlmasPrice() {
        return almasPrice;
    }

    public void setAlmasPrice(double almasPrice) {
        this.almasPrice = almasPrice;
    }
    ...
    public boolean onClick(Player player) {
        ...
        if (moneyPrice > 0) {
            ...
            if (!EconomyBridge.hasMoney(player, moneyPrice, EconomyType.HUESITOS)) {
        ...
        if (almasPrice > 0) {
            ...
            if (!EconomyBridge.hasMoney(player, almasPrice, EconomyType.ALMAS)) {
                player.sendMessage(ChestCommands.getLang().no_almas.replace("{almas}", EconomyBridge.formatMoney(almasPrice)));
                return closeOnClick;
            }
        }
        ...
        if (moneyPrice > 0) {
            if (!EconomyBridge.takeMoney(player, moneyPrice, EconomyType.HUESITOS)) {
        ...
        if (almasPrice > 0) {
            if (!EconomyBridge.takeMoney(player, almasPrice, EconomyType.ALMAS)) {
                player.sendMessage(ChatColor.RED + "Error: the transaction couldn't be executed. Please inform the staff.");
                return closeOnClick;
            }
            changedVariables = true;
