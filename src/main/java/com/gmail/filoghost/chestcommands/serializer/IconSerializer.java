...
public class IconSerializer {
    ...
    public static Icon loadIconFromSection(ConfigurationSection section, String iconName, String menuFileName, ErrorLogger errorLogger) {
        ...
        double almas = section.getDouble(Nodes.ALMAS);
        if (almas > 0.0) {
            icon.setAlmasPrice(almas);
        } else if (price < 0.0) {
            errorLogger.addError("The icon \"" + iconName + "\" in the menu \"" + menuFileName + "\" has a negative ALMAS: " + price);
        }
